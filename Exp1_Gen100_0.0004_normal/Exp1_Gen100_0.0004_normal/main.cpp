...
double n_fund_standardization[generation][50][40];//各檔資金水位
...
int best_generation;
double real_all_trend_ratio[50][generation][10];
...
int real_partical[generation][10][50];
int final_portfolio[50];
int all_buy_paper[10][50];//存可買張數的陣列
int last_test_all_buy_paper[50];
int test_all_buy_paper[50];
int all_share_money[10];//存分配資金的陣列
int test_all_share_money[10];
int compare_share_money[50][generation][10];//存每次實驗每個世代每個粒子的分配資金陣列
int compare_buy_paper[100][10][50];//存每個實驗中的組合股票可買張數
int Gbest_share_money;
int Gbest_buy_paper[50];//Gbest組合中股票可買的張數
double real_all_remain_fund[10][50];
double test_real_all_remain_fund[50];
double compare_remain_fund[100][10][50];
double Gbest_all_remain_fund[50];//Gbest的買完各股剩餘資金
double all_n_fund_standardization[100][50][300];
...
double compare_total_yi[100][50][300];
		...
		for (int j = 0; j < n[i]; j++) {
			...
			for (int k = 0; k < day; k++) {
				...
				if (k == 0)
				{
					...
					all_buy_paper[i][j] = buy_papper;
					all_remain_fund = share_money - one_of_price * buy_papper;//買完各股剩餘資金
					real_all_remain_fund[i][j] = all_remain_fund;//把買完各股剩餘資金存入陣列中進行判斷
		...
		for (int t = 0; t < generation; t++)
		{
			for (int i = 0; i < partical_num; i++)
			{
				if (Gbest_max == real_all_trend_ratio[j][t][i])
				{
					...
					{
						Gbest_buy_paper[k] = compare_buy_paper[t][i][k];//可買張數

						for (int d = 0; d < day; d++)
						{
							Gbest_n_fund_standardization[k][d] = all_n_fund_standardization[t][k][d];
						}
					}
					for (int d = 0; d < day; d++)
					{
						real_total_yi[d] = compare_total_yi[t][i][d];
						Gbest_yi = real_total_yi[day - 1];//最後資金
					}
					for (int s = 0; s < s_stock_index; s++)
					{
						final_portfolio[s] = 0;
						if (real_partical[best_generation - 1][i][s] == 1)
						{
	...
	for (int a = 0; a < file_num; a++)
	{
		...
		for (int j = 0; j < experiment_time; j++)
		{
			...
			for (int t = 0; t < generation; t++)
			{
				...
				for (int i = 0; i < partical_num; i++)
				{
					if (all_max == all_trend_ratio[i])
					{
						for (int k = 0; k < n[i]; k++)
						{
							for (int d = 0; d < day; d++)
							{
								all_n_fund_standardization[t][k][d] = n_fund_standardization[i][k][d];
								compare_total_yi[t][i][d] = yi[i][d];
					...
					for (int s = 0; s < s_stock_index; s++)
					{
						//real_partical[t][i][s] = 0;
						real_partical[t][i][s] = partical[i][s];//如果實驗數超過1,需將陣列改成由實驗存取
																//break;
					}
					...
					for (int k = 0; k < n[i]; k++)
					{
						...
						compare_buy_paper[t][i][k] = all_buy_paper[i][k];
						//cout << compare_buy_paper[87][7][k] << endl;
						compare_remain_fund[t][i][k] = real_all_remain_fund[i][k];
						for (int d = 0; d < day; d++)
						{
			...
			for (int t = 0; t < generation; t++)
			{
				for (int i = 0; i < partical_num; i++)
				{
					if (Gbest_max == real_all_trend_ratio[j][t][i])
					{
						for (int k = 0; k < Gbest_n; k++)
						{
							Gbest_all_remain_fund[k] = compare_remain_fund[t][i][k];
						}
					}
				}
			}
